...
def clean_data(**kwargs):
    ti = kwargs['ti']
    df = ti.xcom_pull(task_ids='import_data', key='df')
...
def process_null_data(**kwargs):
    ti = kwargs['ti']
    df = ti.xcom_pull(task_ids='clean_data', key='df')
    df = pd.read_json(df, orient="records", lines=True)
...
def process_cat_data(**kwargs):
    ti = kwargs['ti']
    df = ti.xcom_pull(task_ids='process_null_data', key='df')
    df = pd.read_json(df, orient="records", lines=True)
...
def process_num_data(**kwargs):
    ti = kwargs['ti']
    df = ti.xcom_pull(task_ids='process_cat_data', key='df')
    df = pd.read_json(df, orient="records", lines=True)
...
def process_time_data(**kwargs):
    ti = kwargs['ti']
    df = ti.xcom_pull(task_ids='process_num_data', key='df')
    df = pd.read_json(df, orient="records", lines=True)

    df_actual_date = pd.DataFrame({'actual_date': pd.to_datetime("2016-10-09") * df.shape[0]})
    print(df_actual_date.shape)
    print(df.shape[0])
    df["EDAD"] = (df['actual_date'] - df['ESTU_FECHANACIMIENTO']).dt.days // 365
...
def split_data(**kwargs):
    ti = kwargs['ti']
    df = ti.xcom_pull(task_ids='process_time_data', key='df')
    df = pd.read_json(df, orient="records", lines=True)
...
def train_model(**kwargs):
    ...
    forest_reg = pickle.dumps(forest_reg)
    ti.xcom_push(key='forest_reg', value=forest_reg)
...
process_time_data_task = PythonOperator(
    task_id='process_time_data',
    python_callable=process_time_data,
    dag=dag,
    provide_context=True
)
...
import_data_task >> clean_data_task >> process_null_data_task >> process_cat_data_task >> process_num_data_task >> process_time_data_task >> split_data_task >> train_model_task >> validate_model_task